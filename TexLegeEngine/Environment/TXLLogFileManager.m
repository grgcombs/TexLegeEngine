...
//  TexLege
//
//  Created by Gregory Combs on 4/13/15.
//  Taken from CocoaLumberjack's CompressingLogFileManager.m
//

#import "TXLLogFileManager.h"
#import <zlib.h>
...
@interface TXLLogFileManager ()

@property (nonatomic,assign,getter=isUpToDate) BOOL upToDate;
@property (nonatomic,assign,getter=isCompressing) BOOL compressing;

@end

@implementation TXLLogFileManager
...
- (void)compressNextLogFile
{
    ...
    NSLogVerbose(@"TXLLogFileManager: compressNextLogFile");
...
{
    NSLogVerbose(@"TXLLogFileManager: compressionDidSucceed: %@", logFile.fileName);
...
{
    NSLogWarn(@"TXLLogFileManager: compressionDidFail: %@", logFile.fileName);
...
{
    NSLogVerbose(@"TXLLogFileManager: didArchiveLogFile: %@", [logFilePath lastPathComponent]);
...
{
    NSLogVerbose(@"TXLLogFileManager: didRollAndArchiveLogFile: %@", [logFilePath lastPathComponent]);
...
- (void)backgroundThread_CompressLogFile:(DDLogFileInfo *)logFile
{
    @autoreleasepool {
        ...
        NSLogInfo(@"TXLLogFileManager: Compressing log file: %@", logFile.fileName);
        ...
        do
        {
            @autoreleasepool {
                ...
                NSLogVerbose(@"TXLLogFileManager: Read %li bytes from file", (long)readLength);

                inputDataSize += (NSUInteger)readLength;
                ...
                uLong prevTotalIn = strm.total_in;
                uLong prevTotalOut = strm.total_out;

                int flush = [inputStream hasBytesAvailable] ? Z_SYNC_FLUSH : Z_FINISH;
                deflate(&strm, flush);

                uLong inputProcessed = strm.total_in - prevTotalIn;
                uLong outputProcessed = strm.total_out - prevTotalOut;

                NSLogVerbose(@"TXLLogFileManager: Total bytes uncompressed: %lu", (unsigned long)strm.total_in);
                NSLogVerbose(@"TXLLogFileManager: Total bytes compressed: %lu", (unsigned long)strm.total_out);
                NSLogVerbose(@"TXLLogFileManager: Compression ratio: %.1f%%",
                ...
                do
                {
                    ...
                    {
                        totalWriteLength += (NSUInteger)writeLength;
                    }
        ...
        if (error)
        {
            ...
            NSLogError(@"TXLLogFileManager: Compression of %@ failed: %@", inputFilePath, error);
            error = nil;
            BOOL ok = [[NSFileManager defaultManager] removeItemAtPath:tempOutputFilePath error:&error];
            if (!ok)
                NSLogError(@"TXLLogFileManager:Failed to clean up %@ after failed compression: %@", tempOutputFilePath, error);
        ...
        else
        {
            ...
            if (!ok)
                NSLogWarn(@"TXLLogFileManager - Warning: failed to remove original file %@ after compression: %@", inputFilePath, error);
